	...
	async function create_new_empty_timetable() {
		...
		if (second_click && parsed_result.isOk()) {
			...
			if (timetable_info.isOk()) {
				...
				const imported = await put_timetable_by_id($token_information.a, tt_info.id, {
...
>
	...
	{#if error}
		<div class="text-error">
			...
			<p>Problem: {error}</p>
			<p>
				Do also note that this feature is currently work in progress. It may have issues reading
				certain NUSMods Share links.
			</p>
		</div>
	{/if}
